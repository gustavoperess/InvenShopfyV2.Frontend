...
const WarehouseList = () => {
  ...
  const handleDocument = (type: string) => {
    ...
    const rows = warehouseData.data.map((item: any) => [
      item.id,
      item.WarehouseName,
      item.warehousePhoneNumber,
      item.warehouseEmail,
      item.warehouseCountry,
      item.warehouseCity,
      item.warehouseZipCode,
      ...
      <div className="invenShopfy-content-area px-4 sm:px-7">
        <div className="invenShopfy-report-area bg-white p-5 sm:p-7 custom-shadow rounded-8 mt-7">
          <div className="invenShopfy-product-top-btn flex flex-wrap gap-5 mb-7">
            <Link className='invenShopfy-btn secondary-btn' href="/warehouse/addWarehouse"><span> <i className="fa-regular fa-circle-plus"></i></span> Add Warehouse</Link>
            <div className="file-import-btn-wrapper">
              <Link className='invenShopfy-btn primary-btn' href="/report/warehousereport"><span><i className="fa-regular fa-file-chart-column"></i></span>Warehouse Report</Link>
            </div>
          </div>
                      ...
                      {(popupState: any) => (
                        <React.Fragment>
                          ...
                          <Menu {...bindMenu(popupState)}>
                            ...
                            <MenuItem onClick={() => { handleRequestSort("quantityOfItems"); popupState.close() }}>Qty</MenuItem>
