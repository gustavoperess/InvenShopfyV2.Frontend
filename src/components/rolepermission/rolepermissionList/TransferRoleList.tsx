...
const DEFAULT_TRANSFER_PERMISSIONS: EntityPermissions[] = [
    { entityType: 'Transfers', permissions: [{ action: 'View', isAllowed: false }, { action: 'Add', isAllowed: false }, { action: 'Update', isAllowed: false }, { action: 'Delete', isAllowed: false }] },
];

const TransferRoleList: React.FC<RoleListProps> = ({ permissionsByEntity, calledItem, onProcessComplete, updatePermissions, setIsReadyToSubmit }) => {

    const [mergedPermissions, setMergedPermissions] = useState(DEFAULT_TRANSFER_PERMISSIONS);
    const [selectAllChecked, setSelectAllChecked] = useState(false);

    useEffect(() => {
        if (!setIsReadyToSubmit) {
            const mergePermissions = () => {
                const updatedPermissions = DEFAULT_TRANSFER_PERMISSIONS.map((defaultEntity) => {
        ...
        <div className="invenShopfy-role-list border-b border-solid border-border flex items-center h-[150px]">
            <div className="invenShopfy-role-left flex items-center">
                <div className="invenShopfy-role-topic">
                    <h5 className="text-[18px] font-semibold text-heading mb-4">Transfers</h5>
            ...
            <div className="invenShopfy-role-right w-full border-s border-solid border-border h-full">
...
export default TransferRoleList;
