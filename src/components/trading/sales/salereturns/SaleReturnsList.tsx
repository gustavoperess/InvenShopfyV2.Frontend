...
const SaleReturnsList = () => {
  ...
  const handleDelete = async () => {
    if (salesReturn > 0) {
      try {
        const result = await deleteSalesReturn(salesReturn).unwrap();
        setOpen(false);
        refetch();
        ...
        if (error?.data?.message) {
          ...
          toast.error("Failed to delete Sales. Please try again later.");
        }
      }
    }
  };
  ...
  const filteredData = salesReturnData?.data.filter((item: any) =>
    ...
    item.returnNote.toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.warehouseName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.returnTotalAmount.toString().toLowerCase().includes(searchQuery.toLowerCase())
  ) || [];
  ...
  const handleDocument = (type: string) => {
    if (!salesReturnData?.data?.length) return;
    ...
    const rows = salesReturnData.data.map((item: any) => [
      item.id,
      item.returnDate,
                      ...
                      <TableContainer>
                        <Table aria-label="enhanced table" size="medium">
                          ...
                          <TableBody>
                            ...
                            ) : salesReturnData?.message === "User is not authorized to do this task" ? (
                              <tr>
                                <td colSpan={14}>
                                  <div className="invenShopfy-loading-container">
