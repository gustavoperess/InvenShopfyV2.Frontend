...
const PurchaseReturnsList = () => {
  ...
  const [purchaseReturn, setPurchaseReturn] = useState<number>(0);
  ...
  const handleOpenDelete = (purchaseReturnId: number) => {
    setPurchaseReturn(purchaseReturnId);
    setOpen(true);
  };
  ...
  const handleSelectAllClick = (checked: boolean) => {
    if (checked) {
      setSelected(purchaseReturnData?.data.map((purchaseReturn: any) => purchaseReturn.id));
    } else {
      setSelected([]);
    }
  };
  ...
  const filteredData = purchaseReturnData?.data.filter((item: any) =>
    item.referenceNumber.toLowerCase().includes(searchQuery.toLowerCase()) || 
    item.remarkStatus.toLowerCase().includes(searchQuery.toLowerCase()) || 
    item.supplierName.toLowerCase().includes(searchQuery.toLowerCase()) || 
    item.totalAmount.toString().toLowerCase().includes(searchQuery.toLowerCase()) || 
  ...
  const handleDocument = (type: string) => {
    if (!purchaseReturnData?.data?.length) return;
    ...
    const rows = purchaseReturnData.data.map((item: any) => [
      item.id,
    ...
    if (type === "csv") {
      ...
      saveAs(blob, "purchase_return.csv");
    } else if (type === "pdf") {
      // Generate PDF
      const doc = new jsPDF();
                      ...
                      {(popupState: any) => (
                        <React.Fragment>
                          ...
                          <MenuItem onClick={() => {handleRequestSort("id"); popupState.close();}}>Sl</MenuItem>
                            ...
                            <MenuItem onClick={() => {handleRequestSort("supplierName"); popupState.close()}}>Biller</MenuItem>
                          </Menu>
                        </React.Fragment>
                      )}
                    </PopupState>
            ...
            <div className="invenShopfy-manageproduct-table-area">
              <div className="invenShopfy-common-mat-list w-full mt-0.5">
                        ...
                        <Table aria-label="enhanced table" size="medium">
                          ...
                          <TableHead className='bg-lightest'>
                            <TableRow>
                              ...
                              <TableCell>
                                <Checkbox
                                  indeterminate={selected.length > 0 && selected.length < purchaseReturnData?.data.length}
                                  checked={purchaseReturnData?.data.length > 0 && selected.length === purchaseReturnData?.data.length}
                            ...
                            {/* Rows */}
                              {purchaseReturnLoading ? (
                                ...
                                <TableRow
                                  ...
                                  hover
                                  onClick={() => handleClick(purchaseReturn.id)}
                                  role="checkbox"
                                  aria-checked={isSelected(purchaseReturn.id)}
                                  selected={isSelected(purchaseReturn.id)}
                                >
                ...
                <div className="invenShopfy-pagination-area">
                  ...
                  <TablePagination
                    component="div"
                    count={purchaseReturnData?.totalCount || 0}
