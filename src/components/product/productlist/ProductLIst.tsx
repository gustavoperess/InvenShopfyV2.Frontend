...
const ProductList = () => {
  ...
  const filteredData = productData?.data.filter((item: any) =>
    ...
    item.productPrice.toString().toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.stockQuantity.toString().toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.subCategories.toLowerCase().includes(searchQuery.toLowerCase()) 
  ) || [];
  ...
  return (
    <>
      <div className="invenShopfy-content-area px-4 sm:px-7">
        <div className="invenShopfy-productlist-area bg-white p-5 sm:p-7 custom-shadow rounded-8 mb-5 mt-7">
          <div className="invenShopfy-product-top-btn flex flex-wrap gap-5 mb-7">
            <Link className='invenShopfy-btn secondary-btn' href="/product/addproduct"><span><i className="fa-regular fa-circle-plus"></i></span>Add Product</Link>
            <div className="file-import-btn-wrapper">
              <Link className='invenShopfy-btn primary-btn' href="/product/importProduct"><span><i className="fa-regular fa-folder-arrow-up"></i></span>Import Product</Link>
            </div>
            <div className="file-import-btn-wrapper">
              <Link className='invenShopfy-btn primary-btn' href="/report/productreport"><span><i className="fa-regular fa-file-chart-column"></i></span>Product Report</Link>
            </div>
          </div>
