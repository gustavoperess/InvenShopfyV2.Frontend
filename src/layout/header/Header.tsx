  ...
  return (
    <>
      <div className="flex justify-between items-center gap-4">
        <div className="invenShopfy-sidebar">
          <button className="invenShopfy-sidebar-icon-bar" onClick={() => setShowingNavigationDropdown(!showingNavigationDropdown)} type="button">
            <i className="fa-solid fa-bars-sort"></i>
          </button>
        </div>
        <div className="invenShopfy-header-content flex items-center justify-end min-h-[70px] w-full">
          <div className="invenShopfy-header-shortmenu pe-5 maxSm:pe-4 border-e items-center flex flex-col justify-center border-borderLight min-h-[70px] border-solid">
            ...
            <div className={`invenShopfy-quick-dropdown ${isQuickDropdown ? 'shortmenu-enable' : ''}`}>
              <QuickMenuDropdown />
            </div>
          </div>
          <div
            className="invenShopfy-header-notify-wrapper px-5 border-e border-solid border-borderLight min-h-[70px] flex items-center">
            <div className="invenShopfy-header-notification pe-5 relative">
              <button type="button" className="noti"><i className="fal fa-bell" onClick={toggleNotificationDropdown}></i></button>
              <div className={`invenShopfy-notify-dropdown ${isNotificationDropdown ? 'notifydropdown-enable' : ''}`}>
                <NotificationDropdown />
              </div>
            </div>
            <div className="invenShopfy-header-email pe-5 relative">
              <button type="button" className="emai" onClick={toggleEmailDropdown}><i className="fal fa-envelope"></i></button>
              <div className={`invenShopfy-notify-dropdown ${isEmailDropdown ? 'emaildropdown-enable' : ''}`}>
                <EmailDropdown />
              </div>
            </div>
            <div className="invenShopfy-header-language flex items-center gap-2 relative" onClick={toggleLanguageDropdown}>
              <div className="invenShopfy-header-language-icon">
                <button type="button" className="text-[18px] text-body leading-none"><i className="fal fa-globe"></i></button>
              </div>
              <div className="invenShopfy-header-language-content maxMd:hidden">
                <ul>
              ...
              <div className={`invenShopfy-quick-dropdown invenShopfy-quick-lang-dropdown ${isLanguageDropdown ? 'langmenu-enable' : ''}`}>
                <LanguageDropdown />
              </div>
            </div>
          </div>
          <div className="invenShopfy-header-profile relative ps-5 flex flex-wrap items-center pe-11 maxMd:pe-0">
            <div className="invenShopfy-header-profile-img w-12 maxMd:me-0 cursor-pointer">
            ...
            <div className="invenShopfy-header-profile-info maxMd:hidden ps-2.5 cursor-pointer">
            ...
            <div className={`invenShopfy-quick-dropdown invenShopfy-quick-pro-dropdown ${isProfileDropdown ? 'promenu-enable' : ''}`}>
              <ul>
      ...
      <div className={`invenShopfy-header-overlay ${isQuickDropdown ? 'shortmenu-enable' : ''}`} onClick={toggleQuickDropdown}></div>
      <div className={`invenShopfy-header-overlay ${isLanguageDropdown ? 'langmenu-enable' : ''}`} onClick={toggleLanguageDropdown}></div>
      <div className={`invenShopfy-header-overlay ${isProfileDropdown ? 'promenu-enable' : ''}`} onClick={toggleProfileDropdown}></div>
      <div className={`invenShopfy-header-overlay ${isNotificationDropdown ? 'notifydropdown-enable' : ''}`} onClick={toggleNotificationDropdown}></div>
      <div className={`invenShopfy-header-overlay ${isEmailDropdown ? 'emaildropdown-enable' : ''}`} onClick={toggleEmailDropdown}></div>
